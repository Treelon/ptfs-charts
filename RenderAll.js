...
// Settings
const settings = {
  inkscapeCommand: 'E:\\Utils\\Inkscape\\bin\\inkscape.com',
  magickCommand: 'E:\\Utils\\ImageMagick\\magick.exe',
  configs: [
    {
      dpi: 192,
      exportDir: '__Results',
...
const { statSync, mkdirSync, readdirSync, existsSync } = require('fs');
...
console.log(
  'Rendering Charts:\n' +
    configs
      .map(
        ({ dpi, exportDir }) => `    Exporting to ${exportDir}/light\n` + `    with dpi ${dpi}\n`
      )
      .join('and\n') +
 ...
 * @param {string} file
 */
const filter = file => file.charAt(0) !== '_' && statSync(`./${file}`).isDirectory();
...
readdirSync('.').forEach(island => {
  ...
  readdirSync(`./${island}`).forEach(airport => {
    ...
    // Charts
    readdirSync(`./${island}/${airport}`).forEach(async chart => {
      if (extname(chart) !== '.svg') return;

      charts.push({
        chartPath: `./${island}/${airport}/${chart}`,
        chartName: chart,
        island: island,
      });
    });
  ...
  configs
    .map(
      ({ dpi, exportDir }) =>
        ...
        charts
          .map((chart, i) => {
            if (!existsSync(`./__Results/light/${chart.island}`))
              mkdirSync(`./__Results/light/${chart.island}`);
            const cn = chart.chartName.split('.')[0];
            const arrivalsFolder = cn.includes('ILS') || cn.includes('RNAV') ? '/Arrivals' : '';
            const departuresFolder = cn.includes('DEPARTURE') ? '/Departures' : '';
            if (arrivalsFolder.length && !existsSync(`./__Results/light/${chart.island}/Arrivals`))
              mkdirSync(`./__Results/light/${chart.island}/Arrivals`);
            if (
              arrivalsFolder.length &&
              !existsSync(`./__Results/light/${chart.island}/Departures`)
            )
              mkdirSync(`./__Results/light/${chart.island}/Departures`);

            return (
              `file-open:${chart.chartPath}; ` +
              `export-filename:${exportDir}/light/${chart.island}${arrivalsFolder}${departuresFolder}/${cn}.png; ` +
              'export-do; ' +
              'file-close;'
            );
          })
...
// Spawn inkscape
const Inkscape = spawn(inkscapeCommand, [`--actions=${actions}`]);
...
Inkscape.on('exit', () => {
  ...
  const resultCount = configs
    .map(
      ({ exportDir }) =>
        readdirSync(`./${exportDir}/light`).filter(file => extname(file) === '.png').length
    )
    .reduce((o, n) => o + n, 0);
  ...
  if (cancelled === true) {
    ...
    console.log(
      '\n\x1b[32mDone Inkscape!\x1b[0m\n' +
        ...
        configs.map(({ exportDir }) => `    Exporting to ${exportDir}/dark\n`).join('and\n')
    );

    const execStr = configs
      .map(
        ({ exportDir }) =>
